...
def attestation_insert(attestation_array):
	...
	for item in attestation_array:
		sql_values = "('{}', {}, {}, '{}', {}, '{}', {}, '{}', '{}')".format(db_fit(item.aggr_bits), item.slot, item.cmt_index, db_fit(item.block_root), item.src_epoch, db_fit(item.src_block), item.trgt_epoch, db_fit(item.trgt_block), item.sign)
