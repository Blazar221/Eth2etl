...
from utils.time_tool import get_timestamp
...
def create_block(json_item):
    ...
    block.block_slot = block_inner['slot']
    block.block_epoch = block.block_slot // SLOT_PER_EPOCH
    block.block_timestamp = get_timestamp(block.block_root)
    ...
    block.proposer_slashings = [json_item_to_proposer_slashings(x)
                      for x in block_inner['body']['proposerSlashings']]
    block.attester_slashings = [json_item_to_attester_slashings(x)
